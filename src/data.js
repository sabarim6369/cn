...
int available[R] = {3, 3, 2}; // Initially available resources
int max[P][R] = {
    {7, 5, 3},
    {3, 2, 2},
    {9, 0, 2},
...
`,
  ...
  {
    id: 9,
    title: "10)File Allocation",
    subdivisions: [
      {
        id: "Linkedfileallocation",
        title: "Linkedfileallocation",
        content: `#include <stdio.h>

int main() {
    int memory[100] = {0}; // 0 = free, 1 = allocated
    int start, length, index, next, block;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (memory[start] == 1) {
            printf("Starting block is already allocated. Try again.\n");
            i--; // Retry this file
            continue;
        }

        memory[start] = 1; // allocate start block
        block = start;

        printf("Blocks allocated for file %d: %d", i + 1, start);
        ...
        for (int j = 1; j < length; j++) {
            ...
            if (next == 100) {
                printf("\nMemory full. Cannot allocate further blocks for file %d.\n", i + 1);
                break;
            }
...
`,
      ...
      {
        id: " Indexed File Allocation ",
        title: " Indexed File Allocation ",
        content: `#include <stdio.h>

int main() {
    int files[50] = {0}; // 0 = free, 1 = allocated
    int n, indexBlock, length;
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < length; j++) {
            ...
            if (files[temp] == 0) {
                ...
                printf("Block %d already allocated. Try this file again.\n", temp);
                // Reset previously marked blocks
        ...
        if (count == length) {
            ...
            printf("File %d Indexed Allocation:\n", i + 1);
...
int main() {
    int memory[100] = {0}; // 0 = free, 1 = allocated
    int start, length, i, j, count, files;

    printf("Enter the number of files: ");
    scanf("%d", &files);

    for (int f = 0; f < files; f++) {
        printf("\nEnter the starting block and the length of file %d: ", f + 1);
        scanf("%d %d", &start, &length);
        ...
        if (count == length) {
            ...
            printf("File %d cannot be allocated. Not enough contiguous space.\n", f + 1);
        }
    }
